...
with fiona.open("/home/knn/Desktop/new_layer.shp", "r") as shapefile:
...
for en, date_ in enumerate(modis_set):
    ...
    if prev != month or en == 0:
        A, B, C, D = [np.zeros((4136, 8019)) for row in range(4)]
    ...
    del  data_inter_mars,data_inter_modis
    del A,B,C,D
    del mars_data
...
for row in df_data['Month']:
    for key in df_data.keys():
        if key not in  ['Month', 'A','B','C','D']:
            with rasterio.open(os.path.join(processing_path, "modis_" + str(key) + "_monthly_cont_mat_" + str(row) + ".tif"), "w",
                               **out_meta) as dest:
